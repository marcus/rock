...
describe('Migration System', () => {
  ...
  describe('MigrationRunner', () => {
    ...
    test('should track executed migrations', async () => {
      ...
      await testDb.run('INSERT INTO migrations (filename) VALUES (?)', ['001_test_migration.sql'])
    ...
    test('should identify pending migrations', async () => {
      ...
      const existing = await testDb.get('SELECT filename FROM migrations WHERE filename = ?', [
        '001_create_sound_packs_sounds_join_table.sql',
      ])

      if (!existing) {
        // Mark one migration as executed
        await testDb.run('INSERT INTO migrations (filename) VALUES (?)', [
          '001_create_sound_packs_sounds_join_table.sql',
        ])
      }
